...
> **СТАТУС 2026-08-20: виконано перше дослідження — чесний негативний результат.**
> Проти профілометра (М-03 + Т1016, 274 пари; `docs/10_profilometer_validation.md`)
> приладовий набір A/B встановити НЕ вдалося: усередині доріг слопи несумісні
> (рестрикція діапазону на рівній М-03), пул відображає лише контраст двох доріг.
> Гейти R²>0.85 / MAE<0.5 не пройдено. Інфраструктура готова (CLI `--iri-psd-A/B`,
> книжкові константи незмінні); потрібно ≥4–5 доріг з різними діапазонами IRI і
> контрольованою швидкістю. Працююча альтернатива вже зараз — корекція зсуву
> Eq.6 (−1.55 м/км, LORO MAE 0.16/0.59) і рангова валідність iri_multi (ρ=0.94
> на Т1016).
...
> **СТАТУС 2026-09-20: паспорт носія підставляється у Eq.4/5/6.** Резолвер
> `metrics/vehicle_params.py` (таблиця відображення = припущення методики з
> провенансом `book`/`assumption`/`default`) пише додаткову колонку
> `iri_multi_vehicle`; `iri_multi` (Eq.6, номінали) не змінено. На парах валідації
> набір Eq.4 для фургона зсув не зменшив, а збільшив (−1.55 → −1.95 м/км, LORO MAE
> гірша на обох дорогах) — книжковий клас носія не заміняє калібрування пари;
> див. `docs/10_profilometer_validation.md`, розділ «Чутливість до набору за класом носія».

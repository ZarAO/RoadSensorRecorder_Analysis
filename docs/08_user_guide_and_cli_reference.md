...
IRI смартфона в кроках 2–7 — сире значення аналізатора з `road_segments.csv`;
підтверджений набір `eq6_bias` на агрегацію не впливає — зсув (`bias_with_ci`)
завжди оцінюється від сирого значення.
...
так лінія лишається читабельною на будь-якому фоні базового шару OpenStreetMap (у темній темі растр інвертується CSS-фільтром).

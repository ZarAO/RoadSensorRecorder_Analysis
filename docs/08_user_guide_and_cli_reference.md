...
2. **confirm** — `POST /coefficient-sets/{id}/confirm`; попередній
   підтверджений набір з тим самим **повним** ключем `(model, vehicle_type,
   phone_model, device_id, vehicle_id)` автоматично архівується — не більше
   одного підтвердженого набору на комбінацію (порівняння NULL-безпечне, тому
   набір з ідентичністю і legacy-набір того ж авто співіснують). Відповідь
   містить `reanalyze_candidates` —
...
на N файлів» обіцяло б файли, яких новий набір ніколи не торкнеться.

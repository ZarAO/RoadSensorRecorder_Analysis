...
У `web_admin/` живе локальна адмінка (FastAPI + SQLite backend, Angular 22 frontend):
завантаження CSV із застосунку, запуск аналізу в черзі, перегляд звітів/графіків/мап,
глобальна Leaflet-мапа всіх проаналізованих зон і дашборд. Запуск описано в
`docs/08_user_guide_and_cli_reference.md` (розділ «Веб-адмінка»).
